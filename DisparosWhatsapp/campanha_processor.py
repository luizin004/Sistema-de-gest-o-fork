#!/usr/bin/env python3
"""
Processador Automático de Campanhas WhatsApp
Este script verifica e envia mensagens para leads pendentes da campanha ativa
"""
...
class CampanhaProcessor:
    ...
    async def get_leads_pendentes(self, campanha_id: str, limit: int = 50) -> List[Dict]:
        """Busca leads pendentes da campanha"""
        try:
            # Como não temos tabela de leads separada, vamos buscar os logs ou histórico
            # Por enquanto, vamos simular que não há leads pendentes
            logger.info(f"Buscando leads pendentes para campanha {campanha_id}")
            return []
    ...
    def personalizar_mensagem(self, template: str, lead: Dict) -> str:
        ...
        for variavel, valor in substituicoes.items():
            mensagem = mensagem.replace(variavel, str(valor))
    ...
    def atualizar_status_lead(self, lead_id: str, status: str, mensagem_enviada: str = None, erro: str = None):
        """Atualiza status do lead no banco"""
        try:
            # Como não temos tabela de leads, vamos apenas registrar no log
            logger.info(f"📝 Status do lead {lead_id}: {status}")
            
            if mensagem_enviada:
                logger.info(f"📝 Mensagem: {mensagem_enviada[:100]}...")
            
            if erro:
                logger.error(f"❌ Erro: {erro}")
            
        except Exception as e:
            logger.error(f"❌ Erro ao registrar status do lead {lead_id}: {e}")

    async def processar_lead(self, nome: str, telefone: str, campanha_id: str = None) -> bool:
        """Processa um lead manualmente (sem tabela de leads)"""
        try:
            # Buscar campanha
            if not campanha_id:
