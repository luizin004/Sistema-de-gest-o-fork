// Script para deploy da Edge Function uazapi-instance-config via MCP
const { createClient } = require('@supabase/supabase-js');

// Configuração
const supabaseUrl = 'https://itescalcmmhhlzsmgdfv.supabase.co';
const supabaseKey = process.env.SUPABASE_SERVICE_ROLE_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Ind0cWhwb3ZqbnRqYmpob2JxdHRrIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsInBpYXQiOjE3NjMxNTA4NDEsImV4cCI6MjA3ODcyNjg0MX0.KwiuX5W-7my-D8ezsy2Xg181FPhGHf3bIN0JywQz0Ts';
...
async function deployFunction() {
  console.log("🚀 Iniciando deploy da Edge Function uazapi-instance-config...");
  
  try {
    // Fazer upload do arquivo
    const { data, error } = await supabase.storage
      .from('edge-functions')
      .upload('uazapi-instance-config/index.ts', edgeFunctionContent, {
    ...
    // Esperar um momento para o processamento
    await new Promise(resolve => setTimeout(resolve, 2000));
    
    // Verificar se a função está disponível
    console.log("🔍 Verificando deploy...");
    
    const testResponse = await fetch(`${supabaseUrl}/functions/v1/uazapi-instance-config/instances`, {
      method: 'GET',
      headers: {
    ...
    if (testResponse.ok) {
      ...
      console.log("❌ Edge Function não respondeu corretamente");
      console.log("📊 Status:", testResponse.status);
